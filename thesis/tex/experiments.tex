\section{\remark{Design of the experiments}}\label{sec:experimentalDesign}
...
\remark{More formally, given a vector $\hat{\mathbf y} \in {\mathcal{R}}^{L}$ of scores assigned to each item, and a
...
%
        ...
        that cannot be easily sped-up (e.g the inference happens \remark{in real-time} on a mobile device, or a small web-server).
    ...
    \subsection{Datasets}\label{subsec:datasets}
        ...
        but approximates it with a negative sampling~\cite{w2v}.\remark{czytelnik nie będzie wiedział o co tutaj chodzi; trzeba to napisać albo bardziej ogólnie, albo wytłumaczyć każdy z pojawiających się tutaj elementów}
        ...
        This last step is exactly the MIPS problem we are discussing in this work. \remark{można byłoby porównać Wasze podejście z HSM}
        ...
        For completeness, we summarize the descriptive statistics of the used datasets in \remark{Table~\ref{tab:dsz}}.
    ...
    \subsection{Parameter grid}\label{subsec:parameterGrid}
        ...
        We provide the exact parameters used in the Appendix \remark{(TODO)???}.

    \subsection{\remark{Amazon Web Services}}
