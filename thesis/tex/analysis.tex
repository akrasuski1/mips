...
\caption{Pareto frontiers for each algorithm. \remark{Na siftsmall nie widać IVF?}}
\end{figure}

The graphs show some similarity --- in all cases \remark{Hierarchical} $K$-Means and IVF algorithms were much better
than the alternatives. ALSH algorithm gave very poor results on Amazon-3M and Wiki-LSHTC
datasets --- apparently the sift\remark{SIFT?} vectors have some regularity that ALSH could exploit better.
...
IVF and \remark{Hierarchical} $K$-Means curves follow each other quite closely --- it could be expected though,
since \remark{Hierarchical} $K$-Means algorithm is generalization of IVF. In almost all places \remark{Hierarchical} $K$-Means are even
...
hierarchy, we split the \remark{Hierarchical} $K$-Means curve into three lines, each corresponding to a single
number of layers (for example, \remark{H?}KMeans-2 means $K$-Means with two layers).
...
(on horizontal axis) than \remark{Hierarchical} $K$-Means when using just a single layer. Both algorithms are
...
In contrast, \remark{Hierarchical} $K$-Means are much better \remark{in terms of what?} when allowed to use two or three layers. The use of hierarchy speeds the calculations to the level of IVF --- for some parameters even surpassing
it, especially for Amazon-3M dataset.

The difference between two-layer and three-layer \remark{Hierarchical} $K$-Means is not as amplified. Since
...
It is also notable that two- and three-layer $K$-Means curves \remark{continue much further left?}
than IVF. Of course, such \remark{speed up} comes with significant impact on accuracy, but
...
in the introduction to the \remark{Hierarchical} $K$-Means algorithm chapter. No matter what precision is, the minimal
...
described in \remark{Chapter} 2.1 --- generally, higher values correspond to wider
trees).
...
This result is consistent with our theoretical calculations in \remark{Chapter} 2.1, which
...
check if this assumption holds by \remark{graphing?plotting} $n_{query}$ vs. time and comparing
