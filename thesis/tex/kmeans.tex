...
quantize \remark{warto wyjaśnić, co oznacza czasownik "quantize" w tym przypadku} them to a smaller set of vectors. This technique assumes that a vector
...
\remark{Powyższy akapit można byłoby ładnie opisać w formalny sposób}
...
useful heuristic while searching: \remark{the original data vector maximizing 
...
each data vector to one of them in such a way, that minimizes sum of \remark{squared
distances: taką metrykę używamy w K-meansach?} of each vector to its centroid. In practice, the clusters (vectors
...
Lloyd's algorithm \remark{Nie używałbym tutaj nazwiska, ponieważ algorytm ten był niezależnie zaproponowany przez wiele osób, w tym również przez Hugo Steinhausa}. It consists of two steps performed alternately a number of
...
The time complexity of this algorithm is the sum of complexities of both loops.
...
but increases probability that the returned vector is the correct \remark{closest?} one. A further
...
\remark{W tekście powinny znaleźć się odwołania do algorytmów, np. "The pseudocode of clustering with Hierarchical $K$ means is given as Algorithm~\ref{alg:h-k-means}.}
...
the ratio of sizes of consecutive layers is $ \sqrt[L+1]{N} $ (\remark{as in algorithm???}),
